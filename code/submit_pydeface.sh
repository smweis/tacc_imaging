#!/bin/bash
# Launcher for run_pydeface.sh. Can be run from any directory: BIDS_DIR is
...
if [ "$#" -lt 2 ]; then
    echo "Usage: code/submit_pydeface.sh <SUBJECT_ID> <SESSION_ID> [--dry-run]"
    exit 1
fi

submit_job pydeface run_pydeface.sh "$@"
